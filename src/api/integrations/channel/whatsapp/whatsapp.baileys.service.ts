      ...
      if (response) {
        ...
        if (messageId) {
          ...
          let message = await this.prismaRepository.message.findFirst({
            where: { key: prismaJsonFilter(['id'], { equals: messageId }) },
          });
          if (isLogicalDeleted) {
    ...
    const count = await this.prismaRepository.message.count({
      where: {
        ...
        AND: [
          ...
          keyFilters?.participant ? { key: prismaJsonFilter(['participant'], { equals: keyFilters.participant }) } : {},
          {
            OR: [
              keyFilters?.remoteJid ? { key: prismaJsonFilter(['remoteJid'], { equals: keyFilters.remoteJid }) } : {},
              keyFilters?.remoteJidAlt ? { key: prismaJsonFilter(['remoteJidAlt'], { equals: keyFilters.remoteJidAlt }) } : {},
            ],
          },
        ],
      },
